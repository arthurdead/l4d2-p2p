...
void CSteamID::SetFromString( const char *pchSteamID, EUniverse eDefaultUniverse )
{
	...
	else
	{
		...
		uint64 unSteamID64 = atoi( pchSteamID );
...
#define k_EAccountTypeAnonUser 10
...
const char * CSteamID::Render() const
{
	...
	{
		V_snprintf( pchBuf, k_cBufLen, "[A:%u:%u:%u]", m_EUniverse, m_unAccountID, m_unAccountInstance );
	}
	else if ( k_EAccountTypeGameServer == m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[G:%u:%u]", m_EUniverse, m_unAccountID );
	}
	else if ( k_EAccountTypeMultiseat == m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[M:%u:%u:%u]", m_EUniverse, m_unAccountID, m_unAccountInstance );
	} 
	else if ( k_EAccountTypePending == m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[P:%u:%u]", m_EUniverse, m_unAccountID );
	} 
	else if ( k_EAccountTypeContentServer == m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[C:%u:%u]", m_EUniverse, m_unAccountID );
	}
	else if ( k_EAccountTypeClan == m_EAccountType )
	{
		// 'g' for "group"
		V_snprintf( pchBuf, k_cBufLen, "[g:%u:%u]", m_EUniverse, m_unAccountID );
	}
	else if ( k_EAccountTypeChat == m_EAccountType )
	{
		if ( m_unAccountInstance & k_EChatInstanceFlagClan )
		{
			V_snprintf( pchBuf, k_cBufLen, "[c:%u:%u]", m_EUniverse, m_unAccountID );
		}
		else if ( m_unAccountInstance & k_EChatInstanceFlagLobby )
		{
			V_snprintf( pchBuf, k_cBufLen, "[L:%u:%u]", m_EUniverse, m_unAccountID );
		}
		else // Anon chat
		{
			V_snprintf( pchBuf, k_cBufLen, "[T:%u:%u]", m_EUniverse, m_unAccountID );
		}
	}
	else if ( k_EAccountTypeInvalid == m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[I:%u:%u]", m_EUniverse, m_unAccountID );
	}
	else if ( k_EAccountTypeIndividual == m_EAccountType )
	{
		if ( m_unAccountInstance != k_unSteamUserDesktopInstance )
			V_snprintf( pchBuf, k_cBufLen, "[U:%u:%u:%u]", m_EUniverse, m_unAccountID, m_unAccountInstance );
		else
			V_snprintf( pchBuf, k_cBufLen, "[U:%u:%u]", m_EUniverse, m_unAccountID );
	}
	else if ( k_EAccountTypeAnonUser == m_EAccountType )
	{
		V_snprintf( pchBuf, k_cBufLen, "[a:%u:%u]", m_EUniverse, m_unAccountID );
	}
	else
	{
		V_snprintf( pchBuf, k_cBufLen, "[i:%u:%u]", m_EUniverse, m_unAccountID );
...
bool CSteamID::BValidExternalSteamID() const
{
	if ( m_EAccountType == k_EAccountTypePending )
		return false;
	if ( m_EAccountType != k_EAccountTypeAnonGameServer && m_EAccountType != k_EAccountTypeContentServer && m_EAccountType != k_EAccountTypeAnonUser )
	{
